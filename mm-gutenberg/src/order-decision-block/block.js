 ...
 */
registerBlockType( 'mm/order-decision-block', {
	title: __( 'MM - Order Decision SmartTag' ),
	description: 'This tag is used to show or hide content based on information associated with the current order. This tag can be used on your site only on a Checkout Form or on a Confirmation core page.',
	icon: {
	...
	category: 'common',
	keywords: [
		__( 'MemberMouse - Order Decision SmartTag' ),
		__( 'membermouse' ),
		__( 'order' ),
		__( 'order decision' ),
		__( 'smarttag' ),
	 ...
	 */
	edit: ( props ) => {
		const className = getBlockDefaultClassName( 'mm/order-decision-block' );
		...
		const ALLOWED_BLOCKS = wp.blocks.getBlockTypes().map( block => block.name ).filter( blockName => blockName !== 'mm/order-decision-block' );
		/*eslint-enable*/
			...
			<InspectorControls>
				<Panel>
					<PanelRow className={ 'mm-gb-panel-row mm-gb-flex-col' }>
						<ExternalLink className={ 'viewDocsLink' } href="https://support.membermouse.com/support/solutions/articles/9000020546-order-decision-smarttag-mm-order-decision-">View Documentation</ExternalLink>
		...
		25 -258 4 -124 8 -305 8 -402 l1 -177 38 -8 c45 -10 327 -10 362 -1 l26 7 -4
		408 c-2 224 -7 438 -12 475 -18 141 -117 280 -243 339 -151 70 -369 78 -553
		19 l-101 -33 -65 26 c-112 45 -385 61 -543 32z" /></g></svg> Order Decision</span>
				<InnerBlocks
