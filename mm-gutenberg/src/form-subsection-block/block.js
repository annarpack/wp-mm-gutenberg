...
registerBlockType( 'mm/form-subsection-block', {
	title: __( 'MM - Form Subsection Block SmartTag' ),
	description: 'Insert Form Subsection SmartTag',
	icon: {
	 ...
	 */
	edit: ( props ) => {
		const className = getBlockDefaultClassName( 'mm/form-subsection-block' );
		...
		const ALLOWED_BLOCKS = wp.blocks.getBlockTypes().map( block => block.name ).filter( blockName => blockName !== 'mm/form-subsection-block' );
		/*eslint-enable*/
		...
		const updateSmartTag = ( props ) => {
			...
			let smartTagOutput = '[MM_Form_Subsection type=\'' + props.attributes.type + '\'';
			...
			<InspectorControls>
				<Panel>
					<PanelBody initialOpen={ true } opened={ ( props.attributes.parentType === 'checkout' ) } >
						<ExternalLink className={ 'viewDocsLink' } href="https://support.membermouse.com/support/solutions/articles/9000020510-form-subsection-smarttag-mm-form-subsection-">View Documentation</ExternalLink>
						<SelectControl label={ 'Form Subsection Type' }
		...
		25 -258 4 -124 8 -305 8 -402 l1 -177 38 -8 c45 -10 327 -10 362 -1 l26 7 -4
		408 c-2 224 -7 438 -12 475 -18 141 -117 280 -243 339 -151 70 -369 78 -553
		19 l-101 -33 -65 26 c-112 45 -385 61 -543 32z" /></g></svg> Form Subsection </span>
